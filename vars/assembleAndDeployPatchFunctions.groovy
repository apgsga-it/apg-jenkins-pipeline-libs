...
def doAssembleAndDeploy(parameter) {
    ...
    parameter.packagers.each{packager ->
        ...
        dir(packager.name) {
            def cmd = "./gradlew clean buildPkg deployPkg -PtargetHost=${packager.targetHost} -PinstallTarget=${parameter.target} -PbuildTyp=CLONED -PbaseVersion=${packager.baseVersion} -PcloneTargetPath=${env.WORKSPACE}/clonedInformation -Dgradle.user.home=${env.GRADLE_USER_HOME_PATH} --info --stacktrace"
