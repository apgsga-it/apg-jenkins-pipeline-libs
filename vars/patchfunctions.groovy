...
def patchBuildsConcurrent(jsonParam, revisionClonedPath) {
    if (javaBuildRequired(jsonParam)) {
        ...
        lock("${jsonParam.target}-Build") {
            commonPatchFunctions.logPatchActivity(jsonParam.patchNumber, jsonParam.target, "build", "started",jsonParam.buildUrl == null ? "Not supported for old Patches" : jsonParam.buildUrl)
            ...
            commonPatchFunctions.logPatchActivity(jsonParam.patchNumber, jsonParam.target, "build", "done", jsonParam.buildUrl == null ? "Not supported for old Patches" : jsonParam.buildUrl)
        }
    }
}
...
def patchBuildDbZip(jsonParam) {
    if (dbBuildRequired(jsonParam)) {
        lock("dbBuild-${jsonParam.target}-Build") {
            commonPatchFunctions.logPatchActivity(jsonParam.patchNumber, jsonParam.target, "db-build", "started", jsonParam.buildUrl == null ? "Not supported for old Patches" : jsonParam.buildUrl)
            coDbModules(jsonParam)
            buildDbZip(jsonParam)
            commonPatchFunctions.logPatchActivity(jsonParam.patchNumber, jsonParam.target, "db-build", "done", jsonParam.buildUrl == null ? "Not supported for old Patches" : jsonParam.buildUrl)
        }
    }
}
