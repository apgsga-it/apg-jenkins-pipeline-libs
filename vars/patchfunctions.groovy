    ...
    lock ("BomUpdate${mavenVersionNumber}") {
        ...
        service.serviceMetaData.packages.each{pack ->
            ...
            dir(pack.packagerName) {
                def cmd = "./gradlew publish -PbomBaseVersion=${bomBaseVersionFor(service)} -PinstallTarget=${target} -PupdateArtifact=${module.groupId}:${module.artifactId}:${mavenVersionNumber} ${env.GRADLE_OPTS}"
...
def publishNewRevisionFor(service,patchNumber,target) {
    ...
    lock("revisionFileOperation") {
        service.serviceMetaData.packages.each{pack ->
            ...
            commonPatchFunctions.coFromBranchCvs(service.serviceMetaData.microServiceBranch,pack.packagerName)
            dir(pack.packagerName) {
                def cmd = "./gradlew clean publish -PnewRevision -PbomBaseVersion=${bomBaseVersionFor(service)} -PinstallTarget=${target} -PpatchFilePath=${env.PATCH_DB_FOLDER}/Patch${patchNumber}.json -PbuildType=PATCH ${env.GRADLE_OPTS}"
